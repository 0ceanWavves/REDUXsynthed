...
let _initCallbacks = [];
let _loadedVersion = null; // Track loaded version

// Initialize Three.js on the client side
async function initThreeJS() {
  // Prevent multiple simultaneous initializations
  if (_isInitializing) {
    console.log("Three.js initialization already in progress, waiting...");
    return _initPromise;
  }
  
  // If already initialized, return the cached instance
  if (_THREE) {
    console.log(`Using cached THREE instance v${_loadedVersion || 'unknown'}`);
    return _THREE;
  }
  ...
  // Create a global flag to prevent other modules from loading Three.js
